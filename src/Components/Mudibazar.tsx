...
/**
 * ক্যাটাগরি অনুযায়ী পণ্য — category=grocery (ডিফল্ট)
 * প্রয়োজনমতো <CategorySection category="vegetables" title="তাজা সবজি" /> দিয়ে বদলাও
...
export default function CategorySection({
  ...
  const { products, loading, error } = useProducts(`category=${category}&limit=8`);

  return (
    <section className="w-full py-10 px-6 md:px-12 bg-white dark:bg-emerald-950">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-end justify-between mb-6 flex-wrap gap-2">
          <div>
            <h2 className="text-2xl font-bold text-emerald-950 dark:text-stone-50">{title}</h2>
            <p className="text-sm mt-1 text-stone-500 dark:text-stone-400">
              প্রতিদিনের প্রয়োজনীয় জিনিস
            </p>
          </div>
        ...
        {loading && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="aspect-square rounded-2xl animate-pulse bg-stone-100 dark:bg-emerald-900"
              />
            ))}
          </div>
        )}
